...
idle(cast, Packet, State = #state{proto_state = ProtoState})
  when is_record(Packet, mqtt_packet) ->
    case process_packet(Packet, State) of
        {ok, NState} ->
...
%% Handle Input
running(cast, Packet, State)
  when is_record(Packet, mqtt_packet) ->
    case process_packet(Packet, State) of
        {ok, NState} ->
            {keep_state, ensure_stats_timer(NState)};
        {error, Reason, NState} ->
            {stop, Reason, NState}
    end;

%% Handle Output
running(info, {deliver, Topic, Msg}, State = #state{proto_state = ProtoState}) ->
    %%TODO: Fixme later...
    Deliveries = drain_deliver({1000, 1000000}, [{publish, Topic, Msg}]),
    case emqx_protocol:deliver(Deliveries, ProtoState) of
        {ok, ProtoState1} ->
            State1 = State#state{proto_state = ProtoState1},
            {keep_state, maybe_gc({deliver, Deliveries}, ensure_stats_timer(State1))};
        {error, Reason} ->
            shutdown(Reason, State)
    end;

%% Start Keepalive
running(info, {keepalive, start, Interval}, State = #state{transport = Transport,
                                                           socket = Socket}) ->
    StatFun = fun() ->
...
running(info, {keepalive, check}, State = #state{keepalive = KeepAlive}) ->
...
running(EventType, Content, State) ->
    ?HANDLE(EventType, Content, State).
...
handle({call, From}, kick, _State) ->
    {stop_and_reply, {shutdown, kicked}, [{reply, From, ok}]};
...
handle(info, {tcp_passive, _Sock}, State) ->
    %% Rate limit here
    ok = activate_socket(State1 = ensure_rate_limit(State)),
    {keep_state, State1};
...
handle(info, {timeout, Timer, emit_stats},
    ...
    case emqx_misc:conn_proc_mng_policy(Limits) of
        ...
        hibernate ->
            ...
            {keep_state, NewState#state{gc_state = GcState1}, hibernate};
        {shutdown, Reason} ->
...
handle(info, {shutdown, discard, {ClientId, ByPid}}, State) ->
    ?WARN("discarded by ~s:~p", [ClientId, ByPid]),
    shutdown(discard, State);

handle(info, {shutdown, conflict, {ClientId, NewPid}}, State) ->
    ?WARN("clientid '~s' conflict with ~p", [ClientId, NewPid]),
...
process_incoming(Data, Packets, State = #state{proto_state = ProtoState,
                                               parse_state = ParseState}) ->
    try emqx_frame:parse(Data, ParseState) of
        ...
        _:Reason:Stk->
            ?ERROR("Parse failed for ~p~nStacktrace:~p~nError data:~p", [Reason, Stk, Data]),
            {stop, Reason}
    end.
...
process_packet(Packet, State = #state{proto_state = ProtoState}) ->
    case emqx_protocol:received(Packet, ProtoState) of
        ...
        {error, Reason, ProtoState1} ->
            {error, Reason, State#state{proto_state = ProtoState1}}
    end.

%%------------------------------------------------------------------------------
%% Drain delivery messages for batch

drain_deliver(undefined, Acc) ->
    Acc;

drain_deliver({Cnt, Oct}, Acc) ->
    drain_deliver(Cnt, Oct, Acc).

drain_deliver(Cnt, Oct, Acc) when Cnt =< 0; Oct =< 0 ->
    lists:reverse(Acc);

drain_deliver(Cnt, Oct, Acc) ->
    receive
        {deliver, Topic, Msg = #message{payload = Payload}} ->
            drain_deliver(Cnt - 1, Oct - iolist_size(Payload), [{publish, Topic, Msg}|Acc])
    after 0 ->
        lists:reverse(Acc)
    end.
...
maybe_gc(_, State = #state{gc_state = undefined}) ->
    State;
maybe_gc({deliver, #message{payload = Payload}}, State) ->
...
reply(From, Reply, State) ->
    {keep_state, State, [{reply, From, info(State)}]}.
