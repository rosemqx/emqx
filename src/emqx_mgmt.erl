...
%% Routes
%%--------------------------------------------------------------------
-spec list_routes() -> list().
list_routes() ->
    case check_row_limit([emqx_route]) of
        false -> throw(max_row_limit);
        ok    -> lists:append([ets:tab2list(emqx_route)])
    end.

% same as looking for emqx_route tab in ets
...
-spec export_blacklist() -> list(list(tuple())).
export_blacklist() ->
    lists:foldl(fun(#banned{who = Who, by = By, reason = Reason, at = At, until = Until}, Acc) ->
                    NWho = case Who of 
                        {ip_address, IP} -> {ip_address, inet:ntoa(IP)};
                        _ -> Who
                    end,
...
-spec check_row_limit(list(atom())) -> ok | false.
...
max_row_limit() ->
    application:get_env(emqx, max_row_limit, ?MAX_ROW_LIMIT).
