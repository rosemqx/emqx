...
get(Key, ResponseBody) ->
   maps:get(Key, jsone:decode(list_to_binary(ResponseBody), [return_maps])).
...
acl_cache(_) ->
    ...
    #{<<"code">> := 0, <<"data">> := Caches} = jsone:decode(list_to_binary(Result), [return_maps]),
    ...
    ?assertMatch(#{<<"code">> := 0}, jsone:decode(list_to_binary(Result2), [return_maps])),
    ...
    #{<<"code">> := 0, <<"data">> := Caches3} = jsone:decode(list_to_binary(Result3), [return_maps]),
...
pubsub(_) ->
    ...
    loop(maps:get(<<"data">>, jsone:decode(list_to_binary(Data1), [return_maps]))),
    ...
    loop(maps:get(<<"data">>, jsone:decode(list_to_binary(Data2), [return_maps]))),
    [ ?assert(receive
    ...
    loop(maps:get(<<"data">>, jsone:decode(list_to_binary(Data3), [return_maps]))).
